    ...
    return communication.queryManager.addQuery(async () => {
      ...
      return (communication.lease = new Promise((resolver, rejecter) => {
        let data = Buffer.alloc(0),
          ...
          timeout = setTimeout(() => {
            console.error(
              `Query ${queryString} has timed out (${queryTimeout} ms).`
            );
  ...
  async openConnection(callback) {
    ...
    connection.on('error', err => {
      ...
      wsconnection.send({
        ...
        log: {
          type: 'error',
          message: `Error while connecting to the serial interface ${
            cfg.host
          }. The controller has lost sight of the acquisition board. You should contact us immediately.`
        }
      });
